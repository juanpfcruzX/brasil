...
function executarParseDoHtml(body) {
    ...
    var resultados = {
        ...
        dataDeAbertura: datas.length > 0 ? datas[0] : null,
        ...
        situacao: {
            cadastral: {
                ...
                data: datas.length > 1 ? datas[1] : null,
            },
            especial: {
                descricao: extrair('SITUA..O ESPECIAL'),
                data: datas.length > 4 ? datas[2] : null
            },
        },
        endereco: {
        ...
        consulta: {
            data: datas.length > 4 ? datas[3] : datas[2],
            hora: body.match(regexes.hora)[0],
...
function obterDados(cnpj, captcha, callback) {
    ...
    jar.setCookie(request.cookie(captcha.sessionId), url);
