const utils                  = require('gammautils'),
      path                   = require('path'),
...
function obterBancosPorNome(nomeBuscado) {
    ...
    bancos.forEach(function(banco) {
        const nomeParaBusca = utils.string.getSearchString(banco.nome);
...
function obterFeriados(uf, dataInicial, dataFinal) {
    ...
    if(typeof uf !== 'undefined') {
        const feriadosEstaduais = require(feriadosEstaduaisHash);
        uf = uf.toLowerCase();
...
function eFeriado(data, uf) {
    const feriadosNacionais = require(feriadosNacionaisArray),
    	eFeriado = false;
