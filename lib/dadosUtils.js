var utils = require('gammautils'),
    ...
    feriadosEstaduaisArray = path.join(__dirname, './dados/feriados-estaduais-fixos-array.json');
...
module.exports.feriadosEstaduaisArray = feriadosEstaduaisArray;
...
var siglas = {
    ...
    //http://pt.wikipedia.org/wiki/%C3%8Dndices_de_infla%C3%A7%C3%A3o_do_Brasil#IGP
};
...
function obterFeriados(params) {
    ...
    if(params.uf !== null){
        var feriadosEstaduais = require(feriadosEstaduaisArray);
        feriadosEstaduais.forEach(function(feriadosPorUF) {        
            if(feriadosPorUF.uf === params.uf) {
                feriadosPorUF.feriados.forEach(function(feriado){
                    bancosEncontrados.push(feriado);
                });                
            }
        });
    }
...
function eFeriado(params){
    var feriadosNacionais = require(feriadosNacionaisArray);
    var uf = params.uf;
    var data = params.data;

    feriadosNacionais.forEach(function(feriado){
        if(feriado.data === formataDataFeriado(data)){
            return true;
        }
    });
    if(params.uf !== null){
