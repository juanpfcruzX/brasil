...
function eTituloDeEleitor(tituloDeEleitor) {
	...
	const base         = tituloDeEleitor.substring(0, 10),
	      primeiraBase = tituloDeEleitor.substring(0, 8),
	      estado       = segundaBase = tituloDeEleitor.substring(8, 10);
...
function eRegistroNacional(rn, tipo){
	...
	if(typeof tipo === 'undefined') {
		...
		const fn = module.exports['e' + capitalize(tipo)];
...
function eMatriz(cnpj) {
	...
	const matches = regexParaMatrizEFilial.exec(formatacoes.removerMascara(cnpj));
...
function eCpf(cpf){
	...
	const segundoResto = mod(base + primeiroDigito, multiplicadores);
	const segundoDigito = segundoResto < 2 ? 0 : 11 - segundoResto;
...
function ePisPasep(pisPasep){
	...
	const base = pisPasep.substring(0,10);
